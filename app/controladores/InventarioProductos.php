<?php
    class InventarioProductos extends Controlador {

        public function __construct() {
            $this->inventarioProductosModelo = $this->modelo('InventarioProducto');
        }

        public function index() {
            //Obtener los productos
            $inventarioProductos = $this->inventarioProductosModelo->obtenerInventarioProductos();
            
            $datos = [
                'inventarioProductos' => $inventarioProductos
            ];

            $this->vista('paginas/inventario/productos/inicio', $datos);
        }
        ...
        public function agregarStock($id) {
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                $datos = [
                    'pkIdProducto' => $id,
                    'stockEntrante' => trim($_POST['stockEntrante'])
                ];

                if ($this->inventarioProductosModelo->actualizarStock($datos)) {
                    redireccionar('inventarioProductos');
                } else {
                    die('Algo salió mal');
                }
            }
